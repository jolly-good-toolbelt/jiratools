...
* run ``jiratool make-config --install`` to install an example config file
  ...
  this script will `not` overwrite it, but raise an error.
  You may use ``--install-if-missing`` to continue without change silently
...
``jiratool make-linked`` makes a new JIRA issue that is linked to an exisiting issue;
...
``jiratool comment`` adds a comment to a JIRA issue.
...
and ``jiratool comment`` will read the comment from stdin instead. Note that if you
use ``-`` interactively, you cannot edit your comment before it is posted.

``jiratool search`` searches JIRA using your JQL query.
The ``jira.config`` file is needed to authenticate to JIRA.
...
``jiratool link`` creates a link between two issues.
...
* ``jiratool make-linked JIRA-1234``
...
* ``jiratool make-linked JIRA-1234 --project OTHER``
